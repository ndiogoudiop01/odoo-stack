...
#  Colonnes :
#     slug  nom  odoo_version  pg_version  domaine  base  port_base  date  repo
#
...
registry_init() {
  ...
  if [ ! -f "${REGISTRY_FILE}" ]; then
    printf '# slug\tnom\todoo\tpg\tdomaine\tbase\tport_base\tcree_le\trepo\n' > "${REGISTRY_FILE}"
  fi
}
...
registry_add() {
  # slug nom odoo pg domaine base port_base repo
  registry_init
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$1" "$2" "$3" "$4" "$5" "$6" "$7" "$(date '+%Y-%m-%d')" "${8:--}" \
    >> "${REGISTRY_FILE}"
}
...
registry_list() {
  registry_init
  printf "%-16s %-24s %-6s %-4s %-32s %-18s %-9s %s\n" \
    "SLUG" "CLIENT" "ODOO" "PG" "DOMAINE" "BASE" "PORTS" "CRÉÉ LE"
