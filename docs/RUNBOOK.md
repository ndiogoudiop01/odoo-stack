...
| `pull access denied` sur l'image de base | serveur non authentifié au registre | `docker login ghcr.io -u <owner>` |
| `manifest unknown` sur l'image de base | image jamais publiée pour cette version | `./base-images/build.sh <version> <édition> --push` |
...
## Changer la version du core Odoo (nouvelle image de base)

Le code Odoo vient de l'image de base construite depuis nos dépôts privés.
Après un push sur la branche de version :

```bash
# 1. reconstruire et publier l'image de base
cd /opt/odoo-stack
./base-images/build.sh 19.0 enterprise --push

# 2. sur chaque client concerné
cd clients/acme
make backup
make pull-base        # docker pull de la nouvelle image
make rebuild          # reconstruit la couche client et redémarre
make upgrade M=all    # si des modules ont changé
```

Sur Coolify / Dokploy : **Redeploy** en cochant le pull des images.

Pour savoir exactement quel commit tourne chez un client :
...
Retour arrière : pointer `ODOO_BASE_IMAGE` sur un tag daté antérieur
(`19.0-enterprise-20260901`) et redéployer.
...
Si la dépendance doit être disponible pour **tous** les clients, ajoutez-la
plutôt dans `base-images/extra-requirements.txt` et reconstruisez les images de
base.
