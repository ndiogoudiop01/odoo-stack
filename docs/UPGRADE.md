...
(`ODOO_VERSION` + `ODOO_BASE_IMAGE`), le code métier est isolé dans
`addons-custom/`, les images de base sont taguées par date, et vous avez une
sauvegarde complète avant chaque étape.

Préalable : l'image de base de la version cible doit exister.
...
# 1. mettre à jour ODOO_VERSION, ODOO_BASE_IMAGE (et POSTGRES_VERSION si la
#    version PG change) dans .env ET dans les variables de la plateforme
...
`ODOO_VERSION` et `ODOO_BASE_IMAGE` sur l'ancien tag daté, redéployer, restaurer
l'archive T0.
